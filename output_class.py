...
class ModelOutput(object):
    ...
    PIXEL_SIZE = 0.1457
    X_CENTER = (21.5 - 21.5) * PIXEL_SIZE #0.35490234894050443 # fitted from
    # the KCWI
    # cube # 21.5
    Y_CENTER = (23.5 - 21.5) * PIXEL_SIZE #0.16776792706671506 # fitted from
    # the KCWI
    # cube # 23.5
    ...
    q_light_1 = lambda _: 0.7878 #np.random.normal(0.921, 0.004)
    ...
    q_light_2 = lambda _: 0.7849 #np.random.normal(0.849, 0.004)
    ...
    def __init__(self, mcmc_chain_dir,
        ...
        self.binned_velocity_dispersion = None
        self.IR_map = None
    ...
    def get_light_profile_mge(self, kwargs_light, e11, e12, e21, e22):
        ...
        #mge_fit_1d(rs_1, flux_r_1, ngauss=40, quiet=True)
        ...
        # qobs_lum = np.ones_like(mge_2[1]) * q_2
    ...
    def compute_jampy_velocity_dispersion(self,
                                        a_ani_min=0.5,
                                        a_ani_max=5,
                                        start_index=0,
                                        num_compute=None,
        ...
        for i in range(start_index, start_index+num_compute):
            ...
            if anisotropy_model == 'Osipkov-Merritt':
                ...
                ani_param = np.random.uniform(0.5, np.sqrt(1+0.5))
                # toss = np.random.uniform(0., 1.)
                # if toss > 0.5:
                #     ani_param = 1. / ani_param
            elif anisotropy_model == 'step':
                ani_param = np.random.uniform([0.5, 0.5],
                                              [np.sqrt(1+0.5), np.sqrt(1+0.5)],
                                              size=2)
                # for k in range(2):
                #     toss = np.random.uniform(0, 1.)
                #     if toss > 0.5:
            ...
            if debug:
                if anisotropy_model == 'constant':
                    ani_param = np.sqrt(1 - 0.25)
                    if set_beta is not None:
                        ani_param = np.sqrt(1 - set_beta)
                elif anisotropy_model == 'Osipkov-Merritt':
                    ani_param = 3.

            self.ani_param.append(ani_param)
            ...
            if is_spherical:
                jam = jam_sph_rms(
                    ...
                    # goodbins=goodbins,
                    # align='sph',
                    beta=None if anisotropy_model=='Osipkov-Merritt' else bs,
                    scale=ani_param*r_eff if
                            anisotropy_model=='Osipkov-Merritt' else None,
                ...
                jam = jam_axi_proj(
                    ...
                    plot=False, pixsize=self.PIXEL_SIZE,#self.PIXEL_SIZE/supersampling_factor,
                    pixang=phi, quiet=1,
                    sigmapsf=sigma_psf, normpsf=norm_psf,
                    moment='zz',
                    #goodbins=goodbins,
            ...
            if voronoi_bins is not None:
                ...
                vel_dis_map, intensity_map = self.bin_map(vel_dis_model,
            ...
            if voronoi_bins is not None:
                ...
                self.IR_map.append(intensity_map)
            else:
                self.model_velocity_dispersion.append(vel_dis_map.flatten())
                self.IR_map.append(intensity_map.flatten())

        self.model_velocity_dispersion = np.array(
            self.model_velocity_dispersion)
        self.IR_map = np.array(self.IR_map)

        return self.model_velocity_dispersion

    def bin_map(self, vel_dis_map, IR_map, voronoi_bins):
        """
    ...
    def bin_all_maps(self, voronoi_bins):
        """
        """
        self.binned_velocity_dispersion = []

        for i in range(self.model_velocity_dispersion.shape[0]):
            self.binned_velocity_dispersion.append(
                self.bin_map(
                    self.model_velocity_dispersion[i],
                    self.IR_map[i],
                    voronoi_bins
                )[0]
            )

        self.binned_velocity_dispersion = np.array(
            self.binned_velocity_dispersion
        )
    ...
    def retrieve_anisotropy_profile(self, Rs,
        ...
        qobs_lum = np.ones_like(surf_lum)
    ...
    def compute_galkin_velocity_dispersion(self,
                                          a_ani_min=0.5,
                                          a_ani_max=5,
                                          start_index=0,
                                          num_compute=None,
        ...
        for i in range(start_index, start_index+num_compute):
            ...
            if anisotropy_model == 'Osipkov-Merritt':
                ani_param = 10**np.random.uniform(np.log10(a_ani_min),
                                              np.log10(a_ani_max)) # a_ani
            elif anisotropy_model == 'constant':
                #a_ani = np.random.uniform(0, 1)
                ani_param = np.random.uniform(0.5, np.sqrt(1 + 0.5))
                beta = 1 - ani_param**2
            ...
            if voronoi_bins is not None or single_slit:
                self.model_velocity_dispersion.append(vel_dis)
                self.IR_map.append(IR_map)
            else:
                self.model_velocity_dispersion.append(vel_dis.flatten())
                self.IR_map.append(IR_map.flatten())
    ...
    def load_intensity_map(self, dir_prefix, dir_suffix,
                                 compute_chunk, total_samples=None):
        """
        Load saved model velocity dispersions.
        ...
        for i in range(int(total_samples / compute_chunk)):
            ...
            if loaded_IR_map == []:
                loaded_IR_map = np.loadtxt(file_path)
            else:
                loaded_IR_map = np.append(loaded_IR_map,
                                           np.loadtxt(file_path),
                                           axis=0)

        assert len(loaded_IR_map) == total_samples

        self.IR_map = loaded_IR_map

    def load_ani_param(self, dir_prefix, dir_suffix,
                   compute_chunk, total_samples=None):
        """
        ...
        for i in range(int(total_samples / compute_chunk)):
            ...
            if loaded_ani_param == []:
                loaded_ani_param = np.loadtxt(file_path)
            else:
                loaded_ani_param = np.append(loaded_ani_param, np.loadtxt(file_path),
                                         axis=0)

        assert len(loaded_ani_param) == total_samples

        self.ani_param = loaded_ani_param
