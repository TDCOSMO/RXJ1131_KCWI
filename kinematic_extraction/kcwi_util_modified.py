...
def Xshooter(name):
    ...
    hdu = fits.open(name)
    ssp = hdu[1].data['Flux']
    h2 = hdu[1].data['Wave'][1] - hdu[1].data['Wave'][0] # in the unit of nm
...
def ppxf_kinematics_RXJ1131_getGlobal_lens_deredshift(libary_dir, degree,
                                                      spectrum_aperture,
                                                      wave_min, wave_max,
                                                      z, noise, FWHM, FWHM_tem,
                                                      templates_name,
                                                      velscale_ratio,
                                                      quasar_spectrum=None,
                                                      global_template_lens=None,
                                                      plot=False, spectrum_perpixel=None,
                                                      save_temp=False, temp_array=None,
    ...
    ind_min = find_nearest(np.exp(logLam1), wave_min*10000)
    ind_max = find_nearest(np.exp(logLam1), wave_max*10000)
    #print(ind_min, ind_max)
    
    mask = goodPixels[goodPixels<ind_max]
    mask = mask[mask>ind_min]
    boolen = ~((2900 < mask) & (mask < 2983))  # mask the Mg II #2956
    mask = mask[boolen]
    boolen = ~((2983 < mask) & (mask < 3031))  # mask the Mg II #3001
    mask = mask[boolen]
    #print(mask)
    ...
    return templates, pp, lamRange1, logLam1, lamRange2, logLam2, galaxy, quasar
...
def poisson_noise(T_exp, gal_lin, std_bk_noise, per_second=False):
    ...
    if np.isnan(sigma2).any():
        print(sigma2[np.isnan(sigma2)], const)
...
def remove_quasar_from_galaxy_deredshift(libary_dir, degree, spectrum_aperture,
    ...
    if plot is not False:
        ...
        if np.random.random_integers(1, 50) == 43:
            plt.plot(lin_axis_sky/10, gal_lin, 'k-', label='data')
            plt.plot(lin_axis_sky/10, best_lin, 'r-', label='best model ('
                                                         'lens+quasar)')
            plt.plot(lin_axis_sky/10, gal_lin - sky_lin, 'm-',
...
def get_voronoi_binning_data(dir, name, name1):
    ...
    output=np.loadtxt(dir +'voronoi_2d_binning_' + name1 + '_output.txt')
    ...
    fits.writeto(dir +'voronoi_binning_' + name1 + '_data.fits', b, overwrite=True)
...
def fitting_deredshift(libary_dir,
                       ...
                       spectrum_perpixel=None, verbose=True):
    if quasar_spectrum == None:
        ...
        ppxf_kinematics_RXJ1131_getGlobal_lens_deredshift(
            ...
            global_template_lens=global_temp,
            plot=plot,
            spectrum_perpixel=spectrum_perpixel, verbose=verbose)
        if plot is True:
            plt.show()
        else:
            plt.clf()
        ...
        sigma_poisson = poisson_noise(T_exp, best_lin,
                                      np.std(noise[int(wave_min*10000):int(wave_max*10000)]),
                                      per_second=True)
...
def kinematics_map(dir, name,radius_in_pixels, VD_name=None, vd_val=None):
    '''
    ...
    if VD_name==None:
        ...
        measurements=np.loadtxt(dir + 'VD_%s.txt' % VD_name)
    # Vel, sigma, dv, dsigma
